 ...
 * Token comes from config.prBot.token OR ORR_PR_TOKEN env var.
 */
...
function getToken(config: Config): string {
  ...
  const envToken = process.env.ORR_PR_TOKEN;
  if (envToken) return envToken;
  if (config.prBot.token) {
    console.warn('[pr-bot] Using token from config.json — prefer ORR_PR_TOKEN env var for security');
    return config.prBot.token;
  }
  throw new Error('No GitHub token configured. Set ORR_PR_TOKEN environment variable.');
}
...
export async function findExistingComment(repo: string, prNumber: number, config: Config): Promise<number | null> {
  ...
  if (!token) throw new Error('No GitHub token configured. Set ORR_PR_TOKEN env var or prBot.token in config.');
